...
class VALUE:
    ...
    # 注意！返回的答案代表的是你填入答案ID的位置，例如返回图片答案是BE，则正确答案ID为下列21342047和21342047
    questionId = '6497041'
    answerId = '21342045,21342047,21342046,21342046,21342047'
...
class Spider:
    # url_getExam = "https://studentexam.zhihuishu.com/studentExam/student/doHomework?recruitId=25779&courseId=2072882&examId=yLZPY92e&uuid=XezvLNk2&studentExamId=5JRGGn34"
    ...
    def __analysis(self, anchors):
        ...
        option.optionsIdStr = VALUE.answerId

        optionSortInfo.append(option)

        self.__fetch_content_getAnswer(optionSortInfo)

    def __fetch_content_getAnswer(self, optionSortInfo):
        param = {
        ...
        res = request.urlopen(rq)
        result = res.read().decode("utf-8")
        self.__analysis_second(result);
        return result;

    # 取回答案后进行数据处理
    def __analysis_second(self, anchors):
        json_data = json.loads(anchors)
        print('【答案搜索成功，感谢使用515code查题助手，注意返回的选项结果是对应你option的位置！！！】')

        for key in json_data['rt']:
            print(' 题号', key, ',请将下面链接复制到游览器打开\n', json_data['rt'][key])
